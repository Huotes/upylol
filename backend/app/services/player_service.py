...
from app.core.exceptions import PlayerNotFoundError, RiotAPIError
...
class PlayerService:
    ...
    async def get_profile(
        ...
        """Fetch full player profile: account + summoner + ranked data."""
        ...
        # Parallel fetch: summoner + league + mastery
        summoner = await self._riot.get_summoner_by_puuid(puuid, platform)
        league_entries = await self._riot.get_league_entries(
        ...
        # Extract ranked solo data
        ranked_solo = next(
            (e for e in league_entries if e["queueType"] == "RANKED_SOLO_5x5"),
            None,
        )
